...
struct Reg
{
  ...
  char *
  retSameType (char *str)
  {
    uint32_t val = atoi (str);
    return retSameType (val);
  }

  char *retSameType (uint32_t val);

  char *TryTransfer (uint32_t val);
};

char *
Reg::TryTransfer (uint32_t val)
{
  char *ret = NULL;
...
{
  void
  ParseFilter (uint32_t arch, fprog *prog)
  {
