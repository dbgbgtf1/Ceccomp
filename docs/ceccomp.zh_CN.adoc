...
NOTE: 从 _4.2 版本_ 开始 `dump` 子命令作为 `trace` 的别名被加入，用以保持和 `seccomp-tools` 的兼容性。
...
2. 一行必须短于 384 *字节*。
3. 一个 _TEXT_ 文件必须短于 16384 行。
4. 一个 _TEXT_ 文件必须小于 4 MiB。

并且对于 asm 和 disasm 来说，有效的伪代码（能被编码或解码为 BPF 的）必须少于等于 4096 条，这是内核规定的。
